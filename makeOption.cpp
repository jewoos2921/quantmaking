...
void make_caplet() {
    ...
    for (i = 0; i < nctime; ++i) { zerorate[i] = linear_interpolation(nrtime, rtime, zrate, ctime[i]); }

    double strike, fixed_vol, day_basis, ini_forward_rate;
    double capprice, floor_price;
    fixed_vol = 0.2;
    strike = 0.063;
    day_basis = 365.0;
    ini_forward_rate = 0.062;

    capprice = cap_floor_price(1, ini_forward_rate, strike, fixed_vol, ncoupon,
...
void make_nelson_siegel() {
    ...
    caltype = 1;
    double *nsrate;
    nsrate = new double[nztime];
    nelson_siegel_zerorate(caltype, nztime, ztime, zrate, nsrate);
    ...
    outf << "=Nelson Siegel로 추정한 현물이자율=" << std::endl;
    outf << "만기 Nelson Siegel 현물이자율" << std::endl;
    for (i = 0; i < nztime; ++i) {
        outf << ztime[i] << " " << nsrate[i] << std::endl;
        outf << std::endl << std::endl;
    }
